...
import {
  ...
  FilePlus, Image as ImageIcon, Undo, Redo
} from 'lucide-react';
...
const COMPONENTS = [
  ...
  },
  {
    ...
    authorName: 'Antigravity',
    category: 'situation',
    thumbnail: '/abc/thumbnails/feedback.png',
    nodes: [
  ...
  },
  {
    ...
    authorName: 'Antigravity',
    category: 'situation',
    thumbnail: '/abc/thumbnails/contract.png',
    nodes: [
      ...
      <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
        ...
        {worldNodes.map((n: any) => {
          ...
          if (type === 'context') {
            ...
            borderRadius = '50%';
            iconColor = '#1a192b';
            borderStyle = data.eventType === 'end' ? `4px solid ${borderColor}` : `2px solid ${borderColor}`;
...
const ComponentModal = React.memo(({ isOpen, onClose, onSelect, lang, nodes, dbComponents, loading, onDeleteComp, onEditAttr, showConfirm, onLoadFullData, isOperational, isMember, roles }: any) => {
  ...
  const [activeTab, setActiveTab] = React.useState<'artifact' | 'situation'>('situation');
  ...
  const filteredComponents = Array.from(new Map([...dbComponents, ...COMPONENTS].map(c => [c.id, c])).values())
    .filter(comp => {
      const cat = comp.category || 'situation'; // Default to situation
...
const ComponentSaveModal = ({ isOpen, onClose, onSave, lang, defaultName = '', initialData = null }: any) => {
  ...
  const [category, setCategory] = useState<'artifact' | 'situation'>('situation');
  const [isSaving, setIsSaving] = useState(false);
  const t = (translations as any)[lang];

  useEffect(() => { if (isOpen) { setName(initialData?.name || defaultName); setDescription(initialData?.description || ''); setCategory('situation'); } }, [isOpen, initialData, defaultName]);
  if (!isOpen) return null;
    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1200, backdropFilter: 'blur(4px)' }}>
      <div style={{ background: '#fff', borderRadius: '12px', width: '400px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
          <button onClick={onClose} disabled={isSaving} style={{ padding: '10px 20px', borderRadius: '6px', border: '1px solid #ddd', background: '#fff', opacity: isSaving ? 0.5 : 1 }}>Cancel</button>
          <button onClick={handleSave} disabled={isSaving || !name.trim()} style={{ padding: '10px 20px', borderRadius: '6px', border: 'none', background: '#1a192b', color: '#fff', opacity: (isSaving || !name.trim()) ? 0.7 : 1, cursor: (isSaving || !name.trim()) ? 'not-allowed' : 'pointer' }}>
            {isSaving ? (lang === 'ja' ? '保存中...' : 'Saving...') : 'Save'}
          </button>
        </div>
      </div>
    </div>
  ...
  const t = (translations as any)[lang];
   ...
   useEffect(() => {
     if (isOpen && comp) {
       ...
       setCategory(comp.category || 'situation');
     }
   }, [isOpen, comp]);
...
const ConfirmModal = React.memo(({ isOpen, title, message, onConfirm, onCancel, onSecondary, confirmLabel, cancelLabel, secondaryLabel, lang }: any) => {
  if (!isOpen) return null;
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 3000, backdropFilter: 'blur(8px)' }}>
      <div style={{ background: '#fff', borderRadius: '16px', width: '100%', maxWidth: '400px', padding: '24px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', gap: '16px', border: '1px solid #eee' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '8px', flexDirection: 'column' }}>
          <div style={{ display: 'flex', gap: '12px' }}>
            <button onClick={onCancel} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid #ddd', background: '#fff', cursor: 'pointer', fontSize: '14px', fontWeight: '600', color: '#666' }}>{cancelLabel || (lang === 'ja' ? 'キャンセル' : 'Cancel')}</button>
            <button onClick={onConfirm} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: 'none', background: '#1a192b', cursor: 'pointer', fontSize: '14px', fontWeight: '600', color: '#fff' }}>{confirmLabel || 'OK'}</button>
          </div>
          {onSecondary && (
            <button onClick={onSecondary} style={{ padding: '12px', borderRadius: '8px', border: '1px solid #4f46e5', background: '#f5f3ff', cursor: 'pointer', fontSize: '14px', fontWeight: '600', color: '#4f46e5' }}>{secondaryLabel || 'Secondary'}</button>
          )}
        </div>
      </div>
    </div>
...
const Sidebar = React.memo(({ onImport, onExport, onNew, onExportImage, onDeleteSelected, showGuide, onToggleGuide, onOpenComponentModal, onSaveToLibrary, onUndo, onRedo, canUndo, canRedo, lang, t, isOperational }: any) => {
    ...
    <aside style={{ width: '240px', borderRight: '1px solid #eee', padding: '24px 16px', background: '#fcfcfc', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      ...
      <div style={{ marginBottom: '24px', display: 'flex', gap: '8px' }}>
        <button 
          ...
          disabled={!canUndo}
          style={{ 
            flex: 1, height: '40px', borderRadius: '8px', border: '1px solid #eee', 
            background: '#fff', color: canUndo ? '#1a192b' : '#ccc', 
        ...
        <button 
          ...
          style={{ 
            flex: 1, height: '40px', borderRadius: '8px', border: '1px solid #eee', 
            background: '#fff', color: canRedo ? '#1a192b' : '#ccc', 
            display: 'flex', alignItems: 'center', justifyContent: 'center', 
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        <div 
          style={nodeItemStyle} draggable onDragStart={(e) => onDragStart(e, 'event')}
          onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.borderColor = '#1a192b'; }}
          onMouseLeave={(e) => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.borderColor = '#e0e0e0'; }}
        >
          <div style={{ width: '40px', height: '40px', background: '#fff', border: '2px solid #1a192b', borderRadius: '50%', marginBottom: '8px' }} />
          {t.nodeEvent}
        </div>
      ...
      <div style={{ flexGrow: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        {activeTab === 'attributes' && selectedEdge && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ padding: '16px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '8px', color: '#64748b', textTransform: 'uppercase' }}>{t.propEdgeThickness}</label>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <input 
                  type="range" min="1" max="10" step="0.5"
                  value={selectedEdge.style?.strokeWidth || 2} 
                  onChange={(e) => onUpdateEdge(selectedEdge.id, { style: { ...selectedEdge.style, strokeWidth: parseFloat(e.target.value) } })} 
                  style={{ flex: 1 }} 
                />
                <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#64748b', minWidth: '35px' }}>{selectedEdge.style?.strokeWidth || 2}px</span>
              </div>
            </div>

            <div style={{ padding: '16px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              <label style={{ display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '8px', color: '#64748b', textTransform: 'uppercase' }}>{t.propEdgePattern}</label>
              <select 
                value={selectedEdge.style?.strokeDasharray || ''}
                onChange={(e) => onUpdateEdge(selectedEdge.id, { style: { ...selectedEdge.style, strokeDasharray: e.target.value } })}
                style={{ width: '100%', padding: '8px', border: '1px solid #cbd5e1', borderRadius: '8px', fontSize: '13px' }}
              >
                <option value="">{t.propEdgePatternSolid}</option>
                <option value="5,5">{t.propEdgePatternDashed}</option>
                <option value="2,2">{t.propEdgePatternDotted}</option>
                <option value="10,5,2,5">{lang === 'ja' ? '一点鎖線' : 'Dash-Dot'}</option>
              </select>
            </div>
        ...
        {activeTab === 'attributes' && node && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {node.type === 'event' && (
              <div style={{ padding: '16px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: 'bold', marginBottom: '8px', color: '#64748b', textTransform: 'uppercase' }}>{t.propEventType}</label>
                <select 
                  value={node.data.eventType || 'start'}
                  onChange={(e) => onUpdate(node.id, { ...node.data, eventType: e.target.value })}
                  style={{ width: '100%', padding: '10px', border: '1px solid #cbd5e1', borderRadius: '8px', fontSize: '14px' }}
                >
                  <option value="start">{t.eventStart}</option>
                  <option value="intermediate">{t.eventIntermediate}</option>
                  <option value="end">{t.eventEnd}</option>
                </select>
              </div>
            )}
            ...
            <div style={{ padding: '16px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              <h4 style={{ fontSize: '11px', fontWeight: 'bold', color: '#1a192b', marginBottom: '12px', textTransform: 'uppercase' }}>
                {t.propLabelPosition}
              </h4>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '10px', color: '#64748b', marginBottom: '4px' }}>{t.propPlacement}</label>
                  <select 
                  ...
                  >
                    <option value="inside">{t.posInside}</option>
                    <option value="outside">{t.posOutside}</option>
                  </select>
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '10px', color: '#64748b', marginBottom: '4px' }}>{t.labelVAlign}</label>
                  <select 
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '10px', color: '#64748b', marginBottom: '4px' }}>{t.labelHAlign}</label>
                  <select 
...
const DnDFlow = React.memo(() => {
  ...
  const [currentSource, setCurrentSource] = useState<any>(null); // { id, name, description, category, type: 'library' | 'file' }
  ...
  const refreshLibrary = useCallback(async () => {
    ...
    try { const res = await fetch('/abc/api/components'); const data = await res.json(); if (Array.isArray(data)) setDbComponents(data); }
    catch (err) { console.error('Library fetch failed:', err); }
    finally { setIsLibraryLoading(false); }
  }, []);

  const saveToLibrary = useCallback(async (saveData: any, overwriteId?: string) => {
    try {
      const res = await fetch('/abc/api/components', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          id: overwriteId || ((typeof window !== 'undefined' && window.crypto && window.crypto.randomUUID) ? window.crypto.randomUUID() : Math.random().toString(36).substring(2, 15) + Date.now().toString(36)), 
          ...saveData, 
          type: 'free', 
          data: { nodes, edges },
      ...
      if (res.ok) { 
        ...
        refreshLibrary(); 
        if (overwriteId) {
          setCurrentSource((prev: any) => ({ ...prev, ...saveData }));
        }
      } else {
        const errData = await res.json().catch(() => ({}));
        alert(lang === 'ja' ? `保存に失敗しました: ${errData.details || res.statusText}` : `Save failed: ${errData.details || res.statusText}`);
      }
    } catch(e) { 
      console.error('Save error:', e);
      alert(lang === 'ja' ? '保存中にエラーが発生しました' : 'An error occurred during save'); 
    }
  }, [nodes, edges, user, lang, refreshLibrary]);

  const triggerSave = useCallback(() => {
    if (currentSource?.type === 'library') {
      setConfirmConfig({
        isOpen: true,
        title: lang === 'ja' ? '保存形式の選択' : 'Save Options',
        message: lang === 'ja' ? `既存のモデル「${currentSource.name}」を上書きしますか？それとも別名で保存しますか？` : `Do you want to overwrite "${currentSource.name}" or save it as a new component?`,
        confirmLabel: lang === 'ja' ? '上書き保存' : 'Overwrite',
        secondaryLabel: lang === 'ja' ? '別名で保存 (Save As)' : 'Save As New',
        cancelLabel: lang === 'ja' ? 'キャンセル' : 'Cancel',
        onConfirm: () => {
          saveToLibrary({ name: currentSource.name, description: currentSource.description, category: currentSource.category }, currentSource.id);
  ...
  }, [currentSource, lang, saveToLibrary]);
  ...
  const onDrop = useCallback((event: React.DragEvent) => {
    ...
    const newNode: Node = { 
      ...
      data: { 
        ...
        labelVAlign: (type === 'event' || type === 'gate') ? 'top' : 'top', // Will be ignored if custom
        labelHAlign: (type === 'event' || type === 'gate') ? 'left' : 'left',
        ...
        eventType: type === 'event' ? 'start' : undefined
      }, 
      width: defaultWidth, 
  ...
  const handleSelectComponent = (comp: any) => {
    ...
    if (isLoad) {
      ...
      setNodes(tNodes);
      setEdges(tEdges);
      setCurrentSource({ id: comp.id, name: comp.name, description: comp.description, category: comp.category, type: 'library' });
    } else {
      takeSnapshot();
    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 64px)', paddingTop: '0px', boxSizing: 'border-box', overflow: 'hidden' }}>
           ...
           input.onchange = (e: any) => {
             ...
             reader.onload = (re: any) => {
               try {
                 ...
                 setNodes(inNodes);
                 setEdges(data.edges || []);
                 setCurrentSource({ type: 'file', fileName: file.name });
      ...
      <div style={{ flex: 1, position: 'relative' }} ref={reactFlowWrapper} onDrop={onDrop} onDragOver={(e: any) => e.preventDefault()}>
        ...
        >
          ...
          <Panel position="bottom-center" style={{ background: 'rgba(255,255,255,0.8)', padding: '4px 12px', borderRadius: '20px', fontSize: '10px', color: '#666', border: '1px solid #eee', marginBottom: '10px' }}>
            Modeler v2.2.21-BETA (rev-ef3b0ca) • © 2026 GGBOF Institute
          </Panel>
        </ReactFlow>
      </div>
      ...
      <PropertiesPanel
        ...
        onUpdate={(id: string, data: any) => {
          // Note: for text inputs, we skip snapshot here to avoid spamming history
          // but for structural changes (inputs/outputs counts) we should take one.
          // For now, let's keep it simple.
          setNodes((nds: any) => nds.map((n: any) => n.id === id ? { ...n, data } : n));
        }}
        onUpdateEdge={(id: string, updates: any) => {
          takeSnapshot();
          setEdges((eds: any) => eds.map((e: any) => e.id === id ? { ...e, ...updates } : e));
        }}
      ...
      <ComponentModal
        ...
        onLoadFullData={async (id: string) => { const res = await fetch(`/abc/api/components?id=${id}`); const data: any = await res.json(); return Array.isArray(data) ? data[0]?.data : data?.data; }}
        ...
        onDeleteComp={async (id: string) => { try { await fetch(`/abc/api/components?id=${id}`, { method: 'DELETE' }); refreshLibrary(); } catch(e) { alert('Delete failed'); } }}
      ...
      <ConfirmModal
        ...
        onCancel={confirmConfig.onCancel || (() => setConfirmConfig((prev: any) => ({ ...prev, isOpen: false })))}
